...
function createResponse(array $pformdata)
{
    $tresponse = <<<RESPONSE
		<section class="panel panel-primary" id="Form Response">
					<h1>Thank You {$pformdata["firstName"]}</h1>
					<h3>Your account has been created. 
                    Thank you for keeping up to date with all things Nintendo Switch!</h3>
					<h3>You will receive weekly updates to {$pformdata["inputEmail"]} </h3>
				</div>
		</section>
RESPONSE;
    return $tresponse;
...
$tindexpage = new MasterPage("User Sign-Up");
